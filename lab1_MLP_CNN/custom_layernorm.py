...
class CustomLayerNormAutograd(nn.Module):
    ...
    def forward(self, input):
        ...
        """
        if input.size(1) == self.n_neurons: # input: n_batch* n_neurons
            mean = torch.mean (input, dim=1, keepdim=True)
            variance = torch.var (input, dim=1, keepdim=True)
            print ("mean",mean.shape, "var", variance.shape, "input", input.shape)

            normalized_x = (input - mean)/ torch.sqrt( variance + self.epsilon )
            out = self.gamma * normalized_x + self.beta
            print ("out",out.shape)

        else:
            print ("the shape of input does not match with: number of datapoints * number of neurons")
